...
def create_features2(data: pd.DataFrame, pattern: str, outname: str):
    """
    Fonction pour créer les nouvelles caractéristiques à partir des données spécifiées
    tel que demandé dans la section Ingénierie des caractéristiques II du Milestone 2 et 
    les sauvegarder dans un fichier csv.
    ...
    feature_data['game_seconds'] = data['prdTime'].apply(
        lambda x: int(x.split(':')[0]) * 60 + int(x.split(':')[1]))

    feature_data['game_period'] = data['prd']
    feature_data['coord_x'] = data['coord_x']
    feature_data['coord_y'] = data['coord_y']
    
    feature_data['shot_distance'] = distance_goal(data['coord_x'], data['coord_y'])

    feature_data['shot_angle'] = angle_goal(data['coord_x'], data['coord_y'])

    feature_data['shot_type'] = data['shotCategory']
    
    feature_data['last_event_type'] = data['last_event_type']
    feature_data['last_event_x'] = data['last_event_x']
    feature_data['last_event_y'] = data['last_event_y']
    feature_data['time_since_last_event'] = data['time_since_last_event']
    feature_data['distance_from_last_event'] = data['distance_from_last_event']
    
    feature_data['rebond'] = (data['last_event_type'] == 'SHOT')

    previous_shot_angle = angle_goal(data['last_event_x'], data['last_event_y'])
    
    feature_data['changement_angle_tir'] = np.where(
        feature_data['rebond'],
        previous_shot_angle + feature_data['shot_angle'],
        0
    )

    feature_data['vitesse'] = data['distance_from_last_event'] / data['time_since_last_event']

    # gestion des cas où time_since_last_event est zéro pour éviter une division par zéro
    feature_data['vitesse'].replace(np.inf, 0, inplace=True)
    feature_data['vitesse'].fillna(0, inplace=True)
    feature_data.dropna(inplace=True)  

    feature_data.to_csv(f'../data/derivatives/{outname}', index=False)