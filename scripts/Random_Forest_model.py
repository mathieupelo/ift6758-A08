...
def visualisation(best_model, y_probs, y_test, X_test):
    ROC_plot(y_test, y_probs[:, 1],best_model)
    Centiles_plot(y_test, y_probs[:, 1],best_model)
    cumulative_centiles_plot(y_test, y_probs[:, 1],best_model)
    calibrate_display(best_model, X_test, y_test, 20, model='Random Forest' )
...
class CometExperiment:
    ...
    def read_data(self):
        X, y = preprocessing(self.path, 'goalFlag')
       ...
       # randomized_search = RandomizedSearchCV(model, param_dist, n_iter=n_iter, cv=cv, scoring='accuracy', random_state=42)
        ...
        with experiment.train():
          ...
          #  best_model = model.best_estimator_
           ...
           # experiment.log_metric("best_score", model.best_score_)
        ...
        experiment.log_model(name="Random Forest", file_or_folder='scripts/Random_Forest_model.pkl')
        experiment.end()
        
        y_test = pd.Series(y_test)

        visualisation(model, y_probs, y_test, X_test)
